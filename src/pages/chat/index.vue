<template>
	<view class="content">
		<view class="msg-content">
			<view v-for="item in chatMsg" :key="item.id"
				:class="item.from !== user ? 'msg-item msg-left' : 'msg-item msg-right'">
				<view class="from">{{ item.from || user }} </view>
				<!-- 文本消息 -->
				<view v-if="item.type === 'txt'" class="msg">{{ item.msg }}  {{item.status ? '成功':'失败'}} </view>
				<!-- 图片消息 -->
		...
		<view>
			<view class="input-content">
				<input class="uni-input" v-model="msg" focus confirm-type="search" @confirm="sendTextMsg"
					placeholder="请输入..." />
			...
			sendTextMsg() {
				for (let i = 0; i <= 50; i++) {
					...
					uni.conn.send(textMsg).then((res) => {
						...
						textMsg.status = false
						this.$store.commit("pushMessage", {
							uid: this.to,
							msg: textMsg,
						});
						console.log('消息发送失败', textMsg)
					});
				}
			}
		}
